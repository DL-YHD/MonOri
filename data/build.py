...
def build_dataset(cfg, transforms, dataset_catalog, is_train=True):
    ...
    '''
    dataset_list = cfg.DATASETS.TRAIN if is_train else cfg.DATASETS.TEST # dataset_list = ('kitti_train',)
    ...
    for dataset_name in dataset_list:
        data = dataset_catalog.get(dataset_name) # {'factory': 'KITTIDataset',
                                                 #  'args': {'root': '/media/yhd/Code/YHD/RTM3D_ULTIMATE/kitti_format/data/kitti/training/'}}
        factory = getattr(D, data["factory"])    # factory = data.datasets.kitti.KITTIDataset
        args = data["args"]
...
def make_data_loader(cfg, is_train=True): # 加载数据
    num_gpus = get_world_size() # 获取线程个数
