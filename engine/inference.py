...
def compute_on_dataset(model, data_loader, device, predict_folder, timer=None, vis=False, test_sets_flag=False,
    ...
    with torch.no_grad():
        for idx, batch in enumerate(tqdm(data_loader)):
            ...
            if vis and (not test_sets_flag):
                show_image_with_boxes(vis_target.get_field('ori_img'), output, vis_target, 
                                      visualize_preds, vis_scores=eval_utils['vis_scores'])
            if vis and test_sets_flag:show_image_with_boxes_test(vis_target.get_field('ori_img'), output, vis_target, visualize_preds)
...
def inference(
    ...
    dis_ious = compute_on_dataset(model, data_loader, device, predict_folder, 
                                  inference_timer, vis, test_sets_eval, eval_score_iou)
    comm.synchronize()
...
def inference_all_depths(
    ...
    for cls_idx, cls in enumerate(important_classes):
        ...
        for thresh in cls_thresh:
            logger.info('{} AP@{:.2f}, {:.2f}, {:.2f}:'.format(cls, thresh, thresh, thresh))
            sort_metric = []
