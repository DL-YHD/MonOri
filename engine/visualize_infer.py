...
def show_image_with_boxes_test(image, output, target, visualize_preds):
	...
	for i in range(box2d.shape[0]):
		...
		img2.draw_text(text='{}, {:.3f}'.format(ID_TYPE_CONVERSION[round(clses[i])], score[i]), position=(int(box2d[i, 0]), int(box2d[i, 1])))
		...
		for i in range(keypoint_i.shape[0]):	
			cv2.circle(img_keypoint, tuple([int(keypoint_i[i][0]), int(keypoint_i[i][1])]), 4, keypoint_colors[i], -1)
...
def show_image_with_boxes(image, output, target, visualize_preds, vis_scores=None):
	...
	for i in range(box2d.shape[0]):
		...
		corners3d = box3d_to_corners(locs[i], dims[i], rotys[i])
		corners_2d, depth = calib.project_rect_to_image(corners3d)
		# img3 = draw_projected_box3d(img3, corners_2d, pre_depths[i], cls=ID_TYPE_CONVERSION[round(clses[i])], color=pred_color, draw_orientation=False, draw_height_text=False, draw_center_line=False, draw_corner=False)
		img3 = draw_projected_box3d(img3, corners_2d, cls=ID_TYPE_CONVERSION[round(clses[i])], color=pred_color, draw_orientation=False, draw_height_text=False, draw_center_line=False, draw_corner=False)
		corners3d_lidar = calib.project_rect_to_velo(corners3d)
	...
	for i in range(num_gt):
		...
		corners_2d, depth = calib.project_rect_to_image(corners3d)
		img3 = draw_projected_box3d(img3, corners_2d, gt_locs[i].numpy(), color=gt_color, draw_orientation=False, draw_corner=False)
