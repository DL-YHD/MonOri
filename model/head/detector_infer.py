...
class PostProcessor(nn.Module):
	def __init__(self, cfg, anno_encoder, key2channel):
		...
		self.det_threshold = cfg.TEST.DETECTIONS_THRESHOLD # 0.2
		self.max_detection = cfg.TEST.DETECTIONS_PER_IMG   # 50		
		self.eval_dis_iou = cfg.TEST.EVAL_DIS_IOUS         # False
		self.eval_depth = cfg.TEST.EVAL_DEPTH			   # False
		...
		self.output_depth = cfg.MODEL.HEAD.OUTPUT_DEPTH # soft
		...
		#=================================================================================#
		# svd 伪逆求解空间位置
		self.svd_depth_with_uncertainty = 'svd_inverse_uncertainty' in self.key2channel.keys
		#=================================================================================#
		# use uncertainty to guide the confidence
		self.uncertainty_as_conf = cfg.TEST.UNCERTAINTY_AS_CONFIDENCE
		
		self.key_num = cfg.DATASETS.KEYPOINTS_NUM
	...
	def forward(self, predictions, targets, features=None, test=False, refine_module=None):
		pred_heatmap, pred_regression = predictions['cls'], predictions['reg'] # [1, 3, 96, 320], [1, 91, 96, 320]
		batch = pred_heatmap.shape[0] # batch = 1 
		...
		scores, indexs, clses, ys, xs = select_topk(heatmap, K=self.max_detection) #(1, 50), (1, 50), (1,50), (1,50), (1,50) 

		pred_bbox_points = torch.cat([xs.view(-1, 1), ys.view(-1, 1)], dim=1) #shape = (50, 2)
		pred_regression_pois = select_point_of_interest(batch, indexs, pred_regression).view(-1, pred_regression.shape[1]) # shape = (50, 91)
		...
		if valid_mask.sum() == 0:
			...
			visualize_preds['keypoints'] = scores.new_zeros(0, self.key_num * 2)
		...
		pred_orientation = torch.cat((pred_regression_pois[:, self.key2channel('ori_cls')], pred_regression_pois[:, self.key2channel('ori_offset')]), dim=1) #(N,16)
		...
		if self.regress_keypoints:
			...
			pred_keypoint_offset = pred_keypoint_offset.view(-1, self.key_num, 2)
			# solve depth from estimated key-points
			pred_keypoints_depths = self.anno_encoder.decode_depth_from_keypoints_batch(pred_keypoint_offset, pred_dimensions, calib) # (N,3) (N,5)

			# ====================================================================================================================== #
			# new added 
			# center_point = pred_bbox_points.unsqueeze(1).repeat(1,self.key_num,1) 
			# pred_keypoints_local_offset = pred_regression_pois[:, self.key2channel('keypoint_local_offset')]
			# pred_keypoints_local_offset = pred_keypoints_local_offset.view(-1, self.key_num, 2)

			center_point = (pred_bbox_points + pred_offset_3D).unsqueeze(1).repeat(1,self.key_num,1)
			pred_2D_keypoint = pred_keypoint_offset + center_point

			# svd depth
			svd_location, pred_depth_by_points_svd_inv, _, _, plane_positions = self.anno_encoder.decode_location_by_keypoints_svd_inverse(pred_orientation, pred_2D_keypoint, pred_dimensions, calib, pad_size, target=targets, mask=valid_mask, phase_flag='Inference') # (objs)
			
			# pred_keypoints_depths =  torch.cat((pred_keypoints_depths, pred_depth_by_points_svd_inv.unsqueeze(1)),dim=1)
			# print(pred_keypoints_depths)
			# ====================================================================================================================== # 
			visualize_preds['keypoints'] = pred_keypoint_offset

		if self.svd_depth_with_uncertainty:
    		# way-1 (add pred_offset_3d)
			# center_point = (pred_bbox_points + pred_offset_3D).unsqueeze(1).repeat(1,self.key_num,1)
			
			# way-2 (no add projection offset)
			# center_point = pred_bbox_points.unsqueeze(1).repeat(1,self.key_num,1)
			
			# pred_2D_keypoint = pred_keypoint_offset + center_point
			# location, pred_depth_by_points_svd_inv = self.anno_encoder.decode_location_by_keypoints_svd_inverse(pred_orientation, pred_2D_keypoint, pred_dimensions, calib, pad_size) # (objs)
			pass
		...
		if self.svd_depth_with_uncertainty:
			pred_svd_inverse_uncertainty = pred_regression_pois[:, self.key2channel('svd_inverse_uncertainty')].exp()
		...
		if self.output_depth == 'direct':
		...
		elif self.output_depth == 'svd_inverse':
			pred_depths = pred_depth_by_points_svd_inv
			if self.svd_depth_with_uncertainty: estimated_depth_error = pred_svd_inverse_uncertainty.squeeze(dim=1)
			...
			elif self.output_depth == 'keypoints_svd':
				pred_depths = pred_keypoints_depths[:, 3]
				if self.keypoint_depth_with_uncertainty: estimated_depth_error = pred_keypoint_uncertainty[:, 3]
			
			elif (self.key_num == 27) and (self.output_depth == 'keypoints_15'):
				pred_depths = pred_keypoints_depths[:, 3]
				if self.keypoint_depth_with_uncertainty: estimated_depth_error = pred_keypoint_uncertainty[:, 3]

			elif (self.key_num == 27) and (self.output_depth == 'keypoints_37'):
				pred_depths = pred_keypoints_depths[:, 4]
				if self.keypoint_depth_with_uncertainty: estimated_depth_error = pred_keypoint_uncertainty[:, 4]	
		...
		# hard ensemble, soft ensemble and simple average
			...
			if self.pred_direct_depth and self.depth_with_uncertainty and self.svd_depth_with_uncertainty:
				# 原版
				...
				# ================================================================================================= # 
				# 直接深度估计 + 关键点 + SVD伪逆
				pred_combined_depths = torch.cat((pred_direct_depths.unsqueeze(1), pred_keypoints_depths, pred_depth_by_points_svd_inv.unsqueeze(1)), dim=1)
				pred_combined_uncertainty = torch.cat((pred_direct_uncertainty, pred_keypoint_uncertainty, pred_svd_inverse_uncertainty), dim=1)
				# ================================================================================================= # 
			else:
		...
		if self.svd_depth_with_uncertainty:
			pass
			# pred_2D_keypoint = pred_keypoint_offset + pred_bbox_points.unsqueeze(1).repeat(1,self.key_num,1) #(objs,2) -> (objs, 10, 2)
			# svd_inverse_location, svd_inverse_depth,_,_ = self.anno_encoder.decode_location_by_keypoints_svd_inverse(pred_orientation, pred_2D_keypoint, pred_dimensions, calib, pad_size, phase_flag='Inference') # (objs)
			
		# ============================================================== # 
		# change dimension back to h,w,l
		pred_dimensions = pred_dimensions.roll(shifts=-1, dims=1) # (v,3)
		# ============================================================== # 
	...
	def evaluate_3D_depths(self, targets, pred_regression):
		...
		pred_keypoints_depths = self.anno_encoder.decode_depth_from_keypoints_batch(pred_keypoint_offset.view(-1, self.key_num, 2), pred_dimensions, targets['calib'])
		
		# ======================================================================================================================= # 
		pred_2D_keypoint = pred_keypoint_offset.view(-1, self.key_num, 2) + target_points.unsqueeze(1).repeat(1,self.key_num,1) #(objs,2) -> (objs, 10, 2)

		svd_inverse_location, svd_inverse_depth,_,_ = self.anno_encoder.decode_location_by_keypoints_svd_inverse(pred_orientation, pred_2D_keypoint, pred_dimensions, calib, pad_size, phase_flag='Inference')
		# ======================================================================================================================= #

		# combined uncertainty
		pred_combined_uncertainty = torch.cat((pred_direct_uncertainty, pred_keypoint_uncertainty, pred_svd_inverse_uncertainty), dim=1)
		pred_combined_depths = torch.cat((pred_direct_depths.unsqueeze(1), pred_keypoints_depths, svd_inverse_depth.unsqueeze(1)), dim=1)
		...
		# pred_keypoints_error = pred_combined_error[:, 1:(pred_combined_error.shape(1) - 1)]

		# =============================================== # 
		pred_svd_inverse_error = pred_combined_error[:, -1]
		# =============================================== # 
	...
	def evaluate_3D_detection(self, targets, pred_regression):
		...
		pred_dimensions_offsets = pred_regression_pois[:, self.key2channel('3d_dim')]
		pred_orientation = torch.cat((pred_regression_pois[:, self.key2channel('ori_cls')], pred_regression_pois[:, self.key2channel('ori_offset')]), dim=1)
		pred_keypoint_offset = pred_regression_pois[:, self.key2channel('corner_offset')].view(-1, self.key_num, 2)
		...
		# =============================================================================== #
		center_point = target_points.unsqueeze(1).repeat(1,self.key_num,1)
		pred_2D_keypoint = pred_keypoint_offset + center_point
		# =============================================================================== #
		if self.output_depth == 'direct':
			...
			location, pred_depth_by_points_svd_inv,_,_ = self.anno_encoder.decode_location_by_keypoints_svd_inverse(pred_orientation, pred_2D_keypoint, pred_dimensions, targets['calib'], targets['pad_size'], phase_flag='Inference')
			pred_depths = pred_depth_by_points_svd_inv
