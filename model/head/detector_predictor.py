...
from model.RefConv import RefConv
...
class _predictor(nn.Module):
    def __init__(self, cfg, in_channels):
        ...
        keypoints_num = cfg.DATASETS.KEYPOINTS_NUM
        ...
        self.head_conv = cfg.MODEL.HEAD.NUM_CHANNEL # 256
        self.keypoint_head_conv = cfg.MODEL.HEAD.NUM_CHANNEL  # 256
        ...
        if self.use_inplace_abn:
            ...
            #     nn.Conv2d(in_channels, self.keypoint_head_conv, kernel_size=3, padding=1, bias=False),
            #     InPlaceABN(self.keypoint_head_conv, momentum=self.bn_momentum, activation=self.abn_activision),
            #     nn.Conv2d(self.keypoint_head_conv, keypoints_num, kernel_size=1, padding=1 // 2, bias=True),
            # )
            

            self.class_head = nn.Sequential(
                # RefConv(in_channels, self.head_conv, kernel_size=3, stride=1, padding=1, groups=1, map_k=3),
                CondConv(in_channels, self.head_conv, kernel_size=3, stride=1, padding=1, bias=False),
                InPlaceABN(self.head_conv, momentum=self.bn_momentum, activation=self.abn_activision),
                CondConv(self.head_conv, classes, kernel_size=1, stride=1, padding=1 // 2, bias=True),
                # nn.Conv2d(self.head_conv, classes, kernel_size=1, padding=1 // 2, bias=True),
                # RefConv(self.head_conv, classes, kernel_size=1, stride=1, padding=1 // 2, groups=1, map_k=1),
            )
            
            self.keypoint_head = nn.Sequential(
                # RefConv(in_channels, self.keypoint_head_conv, kernel_size=3, stride=1, padding=1, groups=1, map_k=3),
                CondConv(in_channels, self.keypoint_head_conv, kernel_size=3, stride=1, padding=1, bias=False),
                InPlaceABN(self.keypoint_head_conv, momentum=self.bn_momentum, activation=self.abn_activision),
                CondConv(self.keypoint_head_conv, keypoints_num, kernel_size=1, stride=1, padding=1 // 2, bias=True),
                # nn.Conv2d(self.keypoint_head_conv, keypoints_num, kernel_size=1, padding=1 // 2, bias=True),
                # RefConv(self.keypoint_head_conv, keypoints_num, kernel_size=1, stride=1, padding=1 // 2, groups=1, map_k=1),
            )
            ...
            # self.keypoint_head = nn.Sequential(
            #     nn.Conv2d(in_channels, self.keypoint_head_conv, kernel_size=3, padding=1, bias=False),
            #     norm_func(self.keypoint_head_conv, momentum=self.bn_momentum), nn.ReLU(inplace=True),
            #     nn.Conv2d(self.keypoint_head_conv, keypoints_num, kernel_size=1, padding=1 // 2, bias=True),
            # )
        ...
        self.keypoint_head[-1].bias.data.fill_(- np.log(1 / cfg.MODEL.HEAD.INIT_P - 1))
        ...
        for idx, regress_head_key in enumerate(self.regression_head_cfg):
            ...
            if self.use_inplace_abn:
                feat_layer = nn.Sequential(
                                    # nn.Conv2d(in_channels, self.head_conv, kernel_size=3, padding=1, bias=False),
                                    CondConv(in_planes=in_channels,out_planes=self.head_conv, kernel_size=3, stride=1, padding=1, bias=False),
                                    InPlaceABN(self.head_conv, momentum=self.bn_momentum, activation=self.abn_activision)
                                    )
            else:
                feat_layer = nn.Sequential(
            ...
            for key_index, key in enumerate(regress_head_key):
                ...
                output_head = CondConv(self.head_conv, key_channel, kernel_size=1, stride=1, padding=1 // 2, bias=True)
    ...
    def forward(self, features, targets):
        # Two head braches(cls, reg)
        b, c, h, w = features.shape # [2, 64, 96, 320]
        # output classification
        feature_cls = self.class_head[:-1](features) # [2, 256, 96, 320]
        output_cls = self.class_head[-1](feature_cls) # [2, 3, 96, 320]
        
        feature_keypoint = self.keypoint_head[:-1](features) # [2, 256, 96, 320]
        output_keypoint_heatmap = self.keypoint_head[-1](feature_keypoint) # [2, 12, 96, 320]
        ...
        output_keypoint_heatmap = sigmoid_hm(output_keypoint_heatmap)
        output_regs = torch.cat(output_regs, dim=1)
        return {'cls': output_cls, 'reg': output_regs, 'keypoint_heatmap': output_keypoint_heatmap}
