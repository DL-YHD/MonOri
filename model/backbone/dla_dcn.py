...
from DSCNet.DSCNet_2D_opensource.Code.DRIVE.S3_DSConv_pro import DSConv_pro
...
class DeformConv(nn.Module):
    def __init__(self, chi, cho):
        ...
        self.actf_2 = nn.Sequential(
            nn.GroupNorm(cho // 4, cho),
            nn.ReLU(inplace=True)
        )
        self.conv = DCN(chi, cho, kernel_size=(3,3), stride=1, padding=1, dilation=1, deformable_groups=1)
        # self.conv = DCN(chi, cho, kernel_size=(3,3), stride=1, padding=1, dilation=1, deform_groups=1)

        self.conv0x = DSConv_pro(chi, cho, kernel_size=3, extend_scope=1, morph=0, if_offset=True, device='cuda')
        self.conv0y = DSConv_pro(chi, cho, kernel_size=3, extend_scope=1, morph=1, if_offset=True, device='cuda')
        # self.conv_last = nn.Conv2d(3*cho, cho, 3, padding=1)

    def forward(self, x):
        x_00_0 = self.conv(x)
        x_0x_0 = self.conv0x(x)
        x_0y_0 = self.conv0y(x)
        # x_0_1  = self.conv_last(torch.cat([x_00_0, x_0x_0, x_0y_0], dim=1))
        x_0_1  = x_00_0 + x_0x_0 + x_0y_0
        x = self.actf(x_0_1)
        return x
