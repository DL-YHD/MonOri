...
class Converter_key2channel(object):
     def __init__(self, keys, channels):
         ...
         self.keys = [key for key_group in keys for key in key_group] # ['2d_dim', '3d_offset', 'corner_offset', 'corner_uncertainty', '3d_dim', 'ori_cls', 'ori_offset', 'depth', 'depth_uncertainty']
         self.channels = [channel for channel_groups in channels for channel in channel_groups] # [4, 2, 20, 3, 3, 8, 8, 1, 1]
     def __call__(self, key):
        # find the corresponding index
        index = self.keys.index(key)
